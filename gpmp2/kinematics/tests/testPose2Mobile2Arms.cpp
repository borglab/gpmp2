...
TEST(Pose2Mobile2Arms, 2linkPlanarExamples) {
  ...
  pJp_exp.clear();
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(0))),
                                 q, 1e-6));
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(1))),
                                 q, 1e-6));
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(2))),
                                 q, 1e-6));
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(3))),
                                 q, 1e-6));
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(4))),
                                 q, 1e-6));
  ...
  q = Pose2Vector(Pose2(1.0, 0, M_PI / 4.0),
                  Vector4(M_PI / 4.0, M_PI / 4.0, M_PI / 4.0, M_PI / 4.0));
  pvec_exp.clear();
  pvec_exp.push_back(Pose3(Rot3::Ypr(M_PI / 4.0, 0, 0), Point3(1.0, 0, 0)));
  pvec_exp.push_back(Pose3(Rot3::Ypr(3.0 * M_PI / 4.0, 0, 0),
                           Point3(1.0, 1.414213562373095, 1.0)));
  pvec_exp.push_back(Pose3(Rot3::Ypr(4.0 * M_PI / 4.0, 0, 0),
                           Point3(0.0, 1.414213562373095, 1.0)));
  pvec_exp.push_back(
      Pose3(Rot3::Ypr(5.0 * M_PI / 4.0, 0, 0), Point3(1.0, 0.0, 3.0)));
  pvec_exp.push_back(
      Pose3(Rot3::Ypr(6.0 * M_PI / 4.0, 0, 0), Point3(1.0, -1.0, 3.0)));
  pJp_exp.clear();
  pJp_exp.push_back(
      numericalDerivativeDynamic(std::function<Pose3(const Pose2Vector&)>(
                                     boost::bind(&fkpose, marm, _1, size_t(0))),
