...
TEST(JointLimitFactorPose2Vector, error) {
  ...
  H_exp = numericalDerivativeDynamic(
      std::function<Vector(const Pose2Vector&)>(
          boost::bind(&JointLimitFactorPose2Vector::evaluateError, factor, _1,
                      boost::none)),
      conf, 1e-6);
...
TEST(JointLimitFactorPose2Vector, optimization_1) {
  ...
  noiseModel::Gaussian::shared_ptr prior_model =
      noiseModel::Isotropic::Sigma(5, 1000);
  Key qkey = Symbol('x', 0);
  Vector5 dlimit = (Vector5() << 0, 0, 0, -5.0, -10.0).finished();
  Vector5 ulimit = (Vector5() << 0, 0, 0, 5, 10.0).finished();
  Vector5 thresh = (Vector5() << 0, 0, 0, 2.0, 2.0).finished();
  ...
  graph.add(
      JointLimitFactorPose2Vector(qkey, cost_model, dlimit, ulimit, thresh));
...
TEST(JointLimitFactorPose2Vector, optimization_2) {
  ...
  EXPECT(assert_equal(conf_limit, results.at<Pose2Vector>(qkey).configuration(),
                      1e-6));
}
