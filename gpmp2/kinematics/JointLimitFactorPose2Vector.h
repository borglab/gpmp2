 ...
 *  @brief apply joint limit to vector configuration space (R(N) part) of SE(2)
 *x R(N)
 *  @author Jing Dong
 *  @date  Aug 20, 2017
 **/
 ...
 * but the input joint limit vector and cost dim still have save dim of SE(2) x
 * R(N) = N+3, since we would like to keep the dims consistent everywhere:
 * factor dim reflect var dim so only last N dims of limit vector and cost dims
 * will be used
 */
class JointLimitFactorPose2Vector
 ...
 public:
   ...
   */
  JointLimitFactorPose2Vector(gtsam::Key poseKey,
  ...
  gtsam::Vector evaluateError(
    ...
    if (H1) *H1 = Matrix::Zero(conf.size() + 3, conf.size() + 3);
    Vector err(conf.size() + 3);
    for (size_t i = 0; i < 3; i++) err(i) = 0.0;
      ...
      if (H1) {
        ...
        err(i + 3) =
            hingeLossJointLimitCost(conf(i), down_limit_(i + 3),
                                    up_limit_(i + 3), limit_thresh_(i + 3));
      }
    }
    return err;
  }
