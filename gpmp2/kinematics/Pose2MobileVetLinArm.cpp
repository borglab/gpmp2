...
/* ************************************************************************** */
Pose2MobileVetLinArm::Pose2MobileVetLinArm(const Arm& arm,
                                           const gtsam::Pose3& base_T_torso,
                                           const gtsam::Pose3& torso_T_arm,
                                           bool reverse_linact)
    : Base(arm.dof() + 4, arm.dof() + 2),
      base_T_torso_(base_T_torso),
      torso_T_arm_(torso_T_arm),
...
void Pose2MobileVetLinArm::forwardKinematics(
  ...
  if (!v && (vx || J_vx_p || J_vx_v))
    throw runtime_error(
        "[Pose2MobileArm] ERROR: only ask for velocity in workspace given "
        "velocity in "
        "configuration space");
  ...
  for (size_t i = 0; i < arm_.dof(); i++) {
    ...
    if (J_px_p) {
      ...
      (*J_px_p)[i + 2].block<6, 4>(0, 0) =
          (armjpx[i].inverse() * arm_base).AdjointMap() * Harm_base;
