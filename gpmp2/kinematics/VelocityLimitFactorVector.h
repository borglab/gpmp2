...
class VelocityLimitFactorVector
 ...
 public:
   ...
   */
  VelocityLimitFactorVector(gtsam::Key poseKey,
    ...
    if ((size_t)vel_limit.size() != cost_model->dim() ||
      ...
      throw std::runtime_error(
          "[VelocityLimitFactorVector] ERROR: limit vector dim does not fit.");
    // velocity limit should > 0
    if (vel_limit.minCoeff() <= 0.0)
      throw std::runtime_error(
          "[VelocityLimitFactorVector] ERROR: velocity limit <= 0.");
  }
