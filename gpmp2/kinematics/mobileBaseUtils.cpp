...
gtsam::Pose3 computeBasePose3(const gtsam::Pose2& base_pose2,
  ...
  if (J) {
    ...
    const gtsam::Matrix3 Hzrot3 =
        gtsam::Rot3::ExpmapDerivative(gtsam::Vector3(0, 0, base_pose2.theta()));
    J->block<3, 1>(0, 2) = Hzrot3.col(2);
    J->block<2, 2>(3, 0) = gtsam::Matrix2::Identity();
  }
  return gtsam::Pose3(
      gtsam::Rot3::Rodrigues(gtsam::Vector3(0, 0, base_pose2.theta())),
...
gtsam::Pose3 liftBasePose3(const gtsam::Pose2& base_pose2, double lift,
  ...
  if (J) {
    ...
    return lift_base_pose.compose(
        computeBaseTransPose3(base_pose2, base_T_trans));
  }
}
