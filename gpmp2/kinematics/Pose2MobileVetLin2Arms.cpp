 ...
 *  @brief Abstract plannar mobile manipulator, Pose2 + 2 arms on vertical
 *linear actuator
 *  @author Jing Dong
 *  @date  Aug 20, 2016
 **/
...
Pose2MobileVetLin2Arms::Pose2MobileVetLin2Arms(const Arm& arm1, const Arm& arm2,
                                               ...
                                               const gtsam::Pose3& torso_T_arm2,
                                               bool reverse_linact)
    : Base(arm1.dof() + arm2.dof() + 4, arm1.dof() + arm2.dof() + 2),
      base_T_torso_(base_T_torso),
      torso_T_arm1_(torso_T_arm1),
      torso_T_arm2_(torso_T_arm2),
  ...
  if (!arm1_.base_pose().equals(Pose3::Identity(), 1e-6) ||
    ...
    cout << "[Pose2MobileVetLin2Arms] WARNING: Arm has non-zero base pose, "
            "this base pose will be override. "
...
void Pose2MobileVetLin2Arms::forwardKinematics(
  ...
  if (!v && (vx || J_vx_p || J_vx_v))
    throw runtime_error(
        "[Pose2MobileVetLin2Arms] ERROR: only ask for velocity in workspace "
        "given velocity in "
        "configuration space");
  ...
  if (J_px_p || J_vx_p || J_vx_v) {
    ...
    tso_base = liftBasePose3(p.pose(), p.configuration()(0), base_T_torso_,
                             reverse_linact_, Htso_base);
    Matrix6 H_tso_comp;
  ...
  for (size_t i = 0; i < arm1_.dof(); i++) {
    ...
    if (J_px_p) {
      ...
      (*J_px_p)[i + 2].block<6, 4>(0, 0) =
          (arm1jpx[i].inverse() * arm1_base).AdjointMap() * Harm1_base;
  ...
  for (size_t i = 0; i < arm2_.dof(); i++) {
    ...
    if (J_px_p) {
      // see compose's jacobian
      (*J_px_p)[i + 2 + arm1_.dof()].block<6, 4>(0, 0) =
          (arm2jpx[i].inverse() * arm2_base).AdjointMap() * Harm2_base;
      (*J_px_p)[i + 2 + arm1_.dof()].block(0, 4 + arm1_.dof(), 6, arm2_.dof()) =
          Jarm2_jpx_jp[i];
    }
  }
}
