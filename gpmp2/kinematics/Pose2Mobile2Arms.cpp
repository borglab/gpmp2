...
Pose2Mobile2Arms::Pose2Mobile2Arms(const Arm& arm1, const Arm& arm2,
    ...
    : Base(arm1.dof() + arm2.dof() + 3, arm1.dof() + arm2.dof() + 1),
      base_T_arm1_(base_T_arm1),
      base_T_arm2_(base_T_arm2),
      arm1_(arm1),
      arm2_(arm2) {
...
void Pose2Mobile2Arms::forwardKinematics(
  ...
  if (!v && (vx || J_vx_p || J_vx_v))
    throw runtime_error(
        "[Pose2Mobile2Arms] ERROR: only ask for velocity in workspace given "
        "velocity in "
        "configuration space");
  ...
  // vx[0] = v(0:1,0); vx[i] = vx[0] + angular x arm_base_pos + arm_base_rot *
  // vx_arm[i-1]
  ...
  for (size_t i = 0; i < arm2_.dof(); i++) {
    ...
    if (J_px_p) {
      // see compose's jacobian
      (*J_px_p)[i + 1 + arm1_.dof()].block<6, 3>(0, 0) =
          (arm2jpx[i].inverse() * arm2_base).AdjointMap() * Harm2_base;
      (*J_px_p)[i + 1 + arm1_.dof()].block(0, 3 + arm1_.dof(), 6, arm2_.dof()) =
          Jarm2_jpx_jp[i];
    }
  }
}
