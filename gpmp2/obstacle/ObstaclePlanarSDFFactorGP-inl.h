 ...
 *  @brief Obstacle avoidance cost factor, for 2D arm, using GP and signed
 *distance field
 *  @author Jing Dong
 *  @date  Nov 15, 2015
 **/
...
gtsam::Vector ObstaclePlanarSDFFactorGP<ROBOT, GPINTER>::evaluateError(
  ...
  if (use_H)
    conf = GPbase_.interpolatePose(conf1, vel1, conf2, vel2, Jconf_c1, Jconf_v1,
                                   Jconf_c2, Jconf_v2);
  else
  ...
  for (size_t sph_idx = 0; sph_idx < robot_.nr_body_spheres(); sph_idx++) {
    ...
    if (H1) {
      Matrix12 Jerr_point;
      const Point2 sph_center_2d(sph_centers[sph_idx].x(),
                                 sph_centers[sph_idx].y());
      err(sph_idx) =
          hingeLossObstacleCost(sph_center_2d, sdf_, total_eps, Jerr_point);
