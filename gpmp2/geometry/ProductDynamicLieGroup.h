...
class ProductDynamicLieGroup : public gtsam::ProductLieGroup<G, H> {
 ...
 public:
  ...
  ProductDynamicLieGroup operator*(const ProductDynamicLieGroup& other) const {
    return ProductDynamicLieGroup(
        gtsam::traits<G>::Compose(this->first, other.first),
  ...
  inline size_t dim1() const {
    return DimensionUtils<G, dimension1>::getDimension(this->first);
  }
  inline size_t dim2() const {
    return DimensionUtils<H, dimension2>::getDimension(this->second);
  }
  ...
  TangentVector localCoordinates(const ProductDynamicLieGroup& g,  //
    ...
    typename gtsam::traits<G>::TangentVector v1 =
        gtsam::traits<G>::Local(this->first, g.first);
    typename gtsam::traits<H>::TangentVector v2 =
        gtsam::traits<H>::Local(this->second, g.second);
    TangentVector v(dim());
    v << v1, v2;
    return v;
  }
 ...
 public:
  ...
  static ProductDynamicLieGroup Expmap(const TangentVector& v,
    ...
    if (dimension1 != Eigen::Dynamic) {
      ...
      throw std::runtime_error(
          "[ProductDynamicLieGroup] ERROR: not implement both dynamic types in "
          "Expmap");
    }
    ...
    if (Hv) {
      ...
      Hv->template topLeftCorner<dimension1, dimension1>(dim1_v, dim1_v) =
          D_g_first;
      Hv->template bottomRightCorner<dimension2, dimension2>(dim2_v, dim2_v) =
          D_h_second;
      return ProductDynamicLieGroup(g, h);
  ...
  static TangentVector Logmap(const ProductDynamicLieGroup& p,
    ...
    if (Hp) {
      ...
      Jacobian2 D_h_second;
      typename gtsam::traits<G>::TangentVector v1 =
          gtsam::traits<G>::Logmap(p.first, D_g_first);
      typename gtsam::traits<H>::TangentVector v2 =
          gtsam::traits<H>::Logmap(p.second, D_h_second);
      ...
      TangentVector v(p.dim());
      v << gtsam::traits<G>::Logmap(p.first),
