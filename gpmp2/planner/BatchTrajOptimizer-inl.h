...
gtsam::Values BatchTrajOptimize(const ROBOT& arm, const SDF& sdf,
  ...
  const double delta_t =
      setting.total_time / static_cast<double>(setting.total_step);
  const double inter_dt =
      delta_t / static_cast<double>(setting.obs_check_inter + 1);
  ...
  for (size_t i = 0; i <= setting.total_step; i++) {
    ...
    // start and end
    if (i == 0) {
      graph.add(PriorFactor<typename ROBOT::Pose>(pose_key, start_conf,
                                                  setting.conf_prior_model));
      graph.add(PriorFactor<typename ROBOT::Velocity>(vel_key, start_vel,
                                                      setting.vel_prior_model));

    } else if (i == setting.total_step) {
      graph.add(PriorFactor<typename ROBOT::Pose>(pose_key, end_conf,
                                                  setting.conf_prior_model));
      graph.add(PriorFactor<typename ROBOT::Velocity>(vel_key, end_vel,
                                                      setting.vel_prior_model));
    }

    if (setting.flag_pos_limit) {
      // joint position limits
      graph.add(LIMIT_FACTOR_POS(
          pose_key, setting.pos_limit_model, setting.joint_pos_limits_down,
          setting.joint_pos_limits_up, setting.pos_limit_thresh));
    }
    if (setting.flag_vel_limit) {
      // velocity limits
      graph.add(LIMIT_FACTOR_VEL(vel_key, setting.vel_limit_model,
                                 setting.vel_limits, setting.vel_limit_thresh));
    }

    // non-interpolated cost factor
    graph.add(
        OBS_FACTOR(pose_key, arm, sdf, setting.cost_sigma, setting.epsilon));
    ...
    if (i > 0) {
      ...
      if (setting.obs_check_inter > 0) {
        for (size_t j = 1; j <= setting.obs_check_inter; j++) {
          ...
          graph.add(OBS_FACTOR_GP(last_pose_key, last_vel_key, pose_key,
                                  vel_key, arm, sdf, setting.cost_sigma,
                                  setting.epsilon, setting.Qc_model, delta_t,
                                  tau));
        }
      }
...
double CollisionCost(const ROBOT& robot, const SDF& sdf,
  ...
  OBS_FACTOR obs_factor =
      OBS_FACTOR(Symbol('x', 0), robot, sdf, setting.cost_sigma, 0);
  for (size_t i = 0; i < result.size() / 2; i++)
    coll_cost += (obs_factor.evaluateError(
                      result.at<typename ROBOT::Pose>(Symbol('x', i))))
                     .sum();
