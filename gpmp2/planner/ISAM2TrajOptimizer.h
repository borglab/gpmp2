...
class ISAM2TrajOptimizer {
 private:
  ...
  gtsam::FactorIndices
      removed_factor_index_;  // factors want to be removed in next update
  size_t goal_conf_factor_idx_,
      goal_vel_factor_idx_;  // index of goal prior factor, use to remove
 ...
 public:
  ...
  void initFactorGraph(const Pose& start_conf, const Velocity& start_vel,
  ...
  void fixConfigAndVel(size_t state_idx, const Pose& conf_fix,
                       const Velocity& vel_fix);

  /// add pose estimate after execution (mean and covariance) at current isam2
  /// values also need current time stamp (state index)
  void addPoseEstimate(size_t state_idx, const Pose& pose,
                       const Matrix& pose_cov);

  /// add state estimate after execution (mean and covariance) at current isam2
  /// values also need current time stamp (state index)
  void addStateEstimate(size_t state_idx, const Pose& pose,
                        const Matrix& pose_cov, const Velocity& vel,
...
typedef internal::ISAM2TrajOptimizer<
    ArmModel, GaussianProcessPriorLinear, PlanarSDF, ObstaclePlanarSDFFactorArm,
    ObstaclePlanarSDFFactorGPArm, JointLimitFactorVector,
