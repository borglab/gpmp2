...
/* ************************************************************************** */
gtsam::Values BatchTrajOptimize2DArm(const ArmModel& arm, const PlanarSDF& sdf,
  ...
  return internal::BatchTrajOptimize<
      ArmModel, GaussianProcessPriorLinear, PlanarSDF,
      ObstaclePlanarSDFFactorArm, ObstaclePlanarSDFFactorGPArm,
...
gtsam::Values BatchTrajOptimize3DArm(
  ...
  return internal::BatchTrajOptimize<
      ArmModel, GaussianProcessPriorLinear, SignedDistanceField,
      ObstacleSDFFactorArm, ObstacleSDFFactorGPArm, JointLimitFactorVector,
      VelocityLimitFactorVector>(arm, sdf, start_conf, start_vel, end_conf,
                                 end_vel, init_values, setting);
}
...
gtsam::Values BatchTrajOptimizePose2MobileArm2D(
    ...
    const gtsam::Values& init_values, const TrajOptimizerSetting& setting) {
  return internal::BatchTrajOptimize<
      Pose2MobileArmModel, GaussianProcessPriorPose2Vector, PlanarSDF,
      ObstaclePlanarSDFFactorPose2MobileArm,
      ObstaclePlanarSDFFactorGPPose2MobileArm, JointLimitFactorPose2Vector,
      VelocityLimitFactorVector>(marm, sdf, start_conf, start_vel, end_conf,
                                 end_vel, init_values, setting);
}
...
gtsam::Values BatchTrajOptimizePose2MobileArm(
    ...
    const gtsam::Values& init_values, const TrajOptimizerSetting& setting) {
  return internal::BatchTrajOptimize<
      Pose2MobileArmModel, GaussianProcessPriorPose2Vector, SignedDistanceField,
      ObstacleSDFFactorPose2MobileArm, ObstacleSDFFactorGPPose2MobileArm,
      JointLimitFactorPose2Vector, VelocityLimitFactorVector>(
      marm, sdf, start_conf, start_vel, end_conf, end_vel, init_values,
      setting);
}
...
gtsam::Values BatchTrajOptimizePose2Mobile2Arms(
    ...
    const gtsam::Values& init_values, const TrajOptimizerSetting& setting) {
  return internal::BatchTrajOptimize<
      Pose2Mobile2ArmsModel, GaussianProcessPriorPose2Vector,
      SignedDistanceField, ObstacleSDFFactorPose2Mobile2Arms,
      ObstacleSDFFactorGPPose2Mobile2Arms, JointLimitFactorPose2Vector,
      VelocityLimitFactorVector>(marm, sdf, start_conf, start_vel, end_conf,
                                 end_vel, init_values, setting);
}
...
gtsam::Values BatchTrajOptimizePose2MobileVetLinArm(
    ...
    const gtsam::Values& init_values, const TrajOptimizerSetting& setting) {
  return internal::BatchTrajOptimize<
      Pose2MobileVetLinArmModel, GaussianProcessPriorPose2Vector,
      SignedDistanceField, ObstacleSDFFactorPose2MobileVetLinArm,
      ObstacleSDFFactorGPPose2MobileVetLinArm, JointLimitFactorPose2Vector,
      VelocityLimitFactorVector>(marm, sdf, start_conf, start_vel, end_conf,
                                 end_vel, init_values, setting);
}
...
gtsam::Values BatchTrajOptimizePose2MobileVetLin2Arms(
    ...
    const gtsam::Values& init_values, const TrajOptimizerSetting& setting) {
  return internal::BatchTrajOptimize<
      Pose2MobileVetLin2ArmsModel, GaussianProcessPriorPose2Vector,
      SignedDistanceField, ObstacleSDFFactorPose2MobileVetLin2Arms,
      ObstacleSDFFactorGPPose2MobileVetLin2Arms, JointLimitFactorPose2Vector,
      VelocityLimitFactorVector>(marm, sdf, start_conf, start_vel, end_conf,
                                 end_vel, init_values, setting);
}

/* ************************************************************************** */
double CollisionCost2DArm(const ArmModel& arm, const PlanarSDF& sdf,
                          const gtsam::Values& result,
                          const TrajOptimizerSetting& setting) {
  return internal::CollisionCost<ArmModel, PlanarSDF,
                                 ObstaclePlanarSDFFactorArm>(arm, sdf, result,
                                                             setting);
}

/* ************************************************************************** */
double CollisionCost3DArm(const ArmModel& arm, const SignedDistanceField& sdf,
                          const gtsam::Values& result,
                          const TrajOptimizerSetting& setting) {
  return internal::CollisionCost<ArmModel, SignedDistanceField,
                                 ObstacleSDFFactorArm>(arm, sdf, result,
                                                       setting);
}

/* ************************************************************************** */
double CollisionCostPose2MobileArm2D(const Pose2MobileArmModel& marm,
                                     const PlanarSDF& sdf,
                                     const gtsam::Values& result,
                                     const TrajOptimizerSetting& setting) {
  return internal::CollisionCost<Pose2MobileArmModel, PlanarSDF,
                                 ObstaclePlanarSDFFactorPose2MobileArm>(
      marm, sdf, result, setting);
}

/* ************************************************************************** */
double CollisionCostPose2MobileBase2D(const Pose2MobileBaseModel& robot,
                                      const PlanarSDF& sdf,
                                      const gtsam::Values& result,
                                      const TrajOptimizerSetting& setting) {
  return internal::CollisionCost<Pose2MobileBaseModel, PlanarSDF,
                                 ObstaclePlanarSDFFactorPose2MobileBase>(
      robot, sdf, result, setting);
}

/* ************************************************************************** */
double CollisionCostPose2MobileBase(const Pose2MobileBaseModel& robot,
...
/* ************************************************************************** */
double CollisionCostPose2MobileArm(const Pose2MobileArmModel& marm,
                                   const SignedDistanceField& sdf,
                                   const gtsam::Values& result,
                                   const TrajOptimizerSetting& setting) {
...
/* ************************************************************************** */
double CollisionCostPose2Mobile2Arms(const Pose2Mobile2ArmsModel& marm,
                                     const SignedDistanceField& sdf,
                                     const gtsam::Values& result,
                                     const TrajOptimizerSetting& setting) {
...
/* ************************************************************************** */
double CollisionCostPose2MobileVetLinArm(const Pose2MobileVetLinArmModel& marm,
                                         const SignedDistanceField& sdf,
                                         const gtsam::Values& result,
                                         const TrajOptimizerSetting& setting) {
...
double CollisionCostPose2MobileVetLin2Arms(
  ...
  return internal::CollisionCost<Pose2MobileVetLin2ArmsModel,
                                 SignedDistanceField,
...
gtsam::Values optimize(const gtsam::NonlinearFactorGraph& graph,
  ...
  if (setting.opt_type == TrajOptimizerSetting::Dogleg) {
    ...
    dynamic_cast<LevenbergMarquardtParams*>(params.get())
