...
class VehicleDynamicsFactorPose2
 ...
 public:
   ...
   */
  VehicleDynamicsFactorPose2(gtsam::Key poseKey, gtsam::Key velKey,
                             double cost_sigma)
  ...
  gtsam::Vector evaluateError(
    ...
    if (H1 || H2) {
      Matrix13 Hp, Hv;
      const double err =
          simple2DVehicleDynamicsPose2(pose, vel.head<3>(), Hp, Hv);
      if (H1) {
        *H1 = Matrix::Zero(1, 3);
        H1->block<1, 3>(0, 0) = Hp;
      }
      ...
      return (Vector(1) << simple2DVehicleDynamicsPose2(pose, vel.head<3>()))
          .finished();
    }
  }
