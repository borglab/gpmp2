...
/* ************************************************************************** */
void convertValuesOpenRavePointer(size_t dof, const gtsam::Values& results,
                                  double* pointer, size_t total_step,
                                  const gtsam::Vector& joint_lower_limit,
  ...
  if (joint_lower_limit.size() != static_cast<int>(dof) ||
    ...
    throw std::runtime_error(
        "[convertValuesOpenRavePointer] ERROR: Joint limit size is different "
        "from DOF.");
  ...
  for (size_t i = 0; i <= total_step; i++) {
    ...
    for (size_t j = 0; j < dof; j++) {
      ...
      if (conf(j) < joint_lower_limit(j))
        pointer[dof * i + j] = joint_lower_limit(j);
      else if (conf(j) > joint_upper_limit(j))
        pointer[dof * i + j] = joint_upper_limit(j);
      else
        pointer[dof * i + j] = conf(j);
      // vel
...
/* ************************************************************************** */
void convertOpenRavePointerValues(size_t dof, gtsam::Values& results,
                                  double* pointer, size_t total_step) {
  results.clear();
