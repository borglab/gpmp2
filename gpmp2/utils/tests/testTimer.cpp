...
noiseModel::Isotropic::shared_ptr model_between =
    noiseModel::Isotropic::Sigma(2, 0.01);
...
TEST(Timer, FixedSize) {
  ...
  for (size_t i = 0; i < 4 * side_size; i++) {
    ...
    init_values.insert(Symbol('x', i),
                       Vector2(r + cos(theta) * r, -r + sin(theta) * r));
  }
  // init_values.print();
  ...
  for (size_t i = 0; i < nr_iter; i++) optimizer.iterate();
...
TEST(Timer, DynamicSize) {
  ...
  // prior
  graph.add(PriorFactor<Vector>(Symbol('x', 0), (Vector(2) << 0, 0).finished(),
                                model_prior));

  // between
  for (size_t i = 0; i < side_size; i++)
    graph.add(BetweenFactor<Vector>(Symbol('x', i), Symbol('x', i + 1),
                                    (Vector(2) << interval, 0).finished(),
                                    model_between));
  for (size_t i = 0; i < side_size; i++)
    graph.add(BetweenFactor<Vector>(
        Symbol('x', i + 1 * side_size), Symbol('x', i + 1 + 1 * side_size),
        (Vector(2) << 0, -interval).finished(), model_between));
  for (size_t i = 0; i < side_size; i++)
    graph.add(BetweenFactor<Vector>(
        Symbol('x', i + 2 * side_size), Symbol('x', i + 1 + 2 * side_size),
        (Vector(2) << -interval, 0).finished(), model_between));
  for (size_t i = 0; i < side_size - 1; i++)
    graph.add(BetweenFactor<Vector>(
        Symbol('x', i + 3 * side_size), Symbol('x', i + 1 + 3 * side_size),
        (Vector(2) << 0, interval).finished(), model_between));
  // last one: close loop
  graph.add(BetweenFactor<Vector>(
      Symbol('x', 4 * side_size - 1), Symbol('x', 0),
      (Vector(2) << 0, interval).finished(), model_between));
  ...
  for (size_t i = 0; i < 4 * side_size; i++) {
    ...
    init_values.insert(
        Symbol('x', i),
        (Vector(2) << r + cos(theta) * r, -r + sin(theta) * r).finished());
  }
  // init_values.print();
