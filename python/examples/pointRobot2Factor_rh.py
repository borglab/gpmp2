...
class Parameters(object):  # TODO: read from yaml file or rosparams
    ...
    # prior to start/goal
    pose_fix = pose_fix_model = noiseModel_Isotropic.Sigma(2, 0.0001)
    vel_fix = vel_fix_model = noiseModel_Isotropic.Sigma(2, 0.0001)
...
def main():
    ...
    # start and end conf
    start_conf_val = curstate_val = np.asarray([0, 0])
    start_vel = curstate_vel = np.asarray([0, 0])
    ...
    for i in range(params.total_time_step + 1):
        ...
        conf = result.atVector(symbol(ord("x"), i))
        print(conf, result.atVector(symbol(ord("v"), i)))
