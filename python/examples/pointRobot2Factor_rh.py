...
def get_plan(start_conf_val, start_vel, end_conf_val, end_vel, sdf, params):
    """Get the robot plan given start and end states."""
    ...
    for i in range(0, params.total_time_step + 1):
        ...
        # pose = Pose2(start_conf_val * float(params.total_time_step - i) /
        #              float(params.total_time_step) +
        #              end_conf_val * i / float(params.total_time_step))
        pose = start_conf_val
        vel = avg_vel
        ...
        # graph.add(VehicleDynamicsFactorPose2(key_pos, key_vel, 0.001))
...
class Parameters(object):
    ...
    # prior to start/goal
    pose_fix = pose_fix_model = noiseModel.Isotropic.Sigma(2, 0.0001)
    vel_fix = vel_fix_model = noiseModel.Isotropic.Sigma(2, 0.0001)
...
def main():
    ...
    # start and end conf
    curstate_val = np.asarray([0, 0])
    curstate_vel = np.asarray([0, 0])
    end_conf_val = np.asarray([17, 14])
    ...
    while np.linalg.norm(curstate_val -
        ...
        ## Plots
        plotPointRobot2D(figure, axis, params.pR_model, curstate_val)
    ...
    for i in range(params.total_time_step + 1):
        ...
        conf = result.atVector(X(i))
        print(conf, result.atVector(V(i)))
        plotPointRobot2D(figure, axis, params.pR_model, conf)
