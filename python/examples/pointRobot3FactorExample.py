...
for i in range(0, total_time_step + 1):
    ...
    pose = Pose2(start_conf_val * float(total_time_step - i) /
                 float(total_time_step) +
    ...
    if i > 0:
        ...
        temp = GaussianProcessPriorPose2(key_pos1, key_vel1, key_pos2,
                                         key_vel2, delta_t, Qc_model)
        graph.push_back(temp)

        #% cost factor
        graph.push_back(
            ObstaclePlanarSDFFactorPose2MobileBase(key_pos, pR_model, sdf,
...
for i in range(total_time_step + 1):
    ...
    plotPointRobot2D_theta(
        figure, axis, pR_model,
